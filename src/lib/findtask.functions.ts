...
// ---- Accept tasker (PUT) -----------------------------------------------
// Backend returns `payment_link` (Flutterwave checkout) when funding via deposit.
// When funding from wallet, it returns `funded_from_wallet: true` and no payment_link.
export const acceptApplicant = createServerFn({ method: "POST" })
  .inputValidator((i: unknown) =>
    z.object({
      taskId: TaskId,
